...
wrld_simpl <- project(wrld_simpl,"EPSG:4326")
...
rm(list = c("BIOMES","ModelsAll","HistFiles1980_2005"));gc()
for (Model in c(ModelsAll)){#(Model <- ModelsAll[[1]])
  # Create a climate normal raster for each evaluated variable  
                      ...
                      if(length(grep(Model,dir(paste0("./Data/CMIP5/Processed/",RCP,"/BIOCLIM"))))!=0){
                        ...
                        NoveltyPerPer <- lapply(seq(2099,2299,by=50),
                                                function(YearUse){#(YearUse <- seq(2099,2299,by=50)[5])
                                                  ...
                                                  MDMinDistinKm <- c(rast(ClimNormMn),rast(ClimNormMn[[1]])) # Make am empty SpatRaster File to summarize values
                                                  ...
                                                  MDMinDistinKm <- app(MDMinDistinKm,
                                                                       function(x){#(x<-values(MDMinDistinKm)[1000,])
                                                                         if(!is.na(x[3])){
                                                                           Dist <- as.numeric(min(distance(x = as.matrix(t(x[1:2])),
