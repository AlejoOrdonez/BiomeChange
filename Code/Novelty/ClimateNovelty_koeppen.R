...
for (Model in c(ModelsAll)){#(Model <- ModelsAll[[1]])
  ...
  for (RCP in c("RCP26", "RCP46", "RCP60", "RCP85")){#(RCP <- c("RCP26", "RCP46", "RCP60", "RCP85")[1])
    if(length(grep(Model,dir(paste0("./Data/CMIP5/Processed/",RCP,"/koeppen_geiger"))))!=0){
      ...
      names(ClimNormSD) <- names(RCPList[[1]])
      #Crop oceans
      ...
      rm(list = ls()[!ls()%in%c("Model","RCP","BIOMES","ModelsAll","HistFiles1980_2005","MDtresh","SEDtresh",
                                "ClimNormMn","ClimNormSD")]);gc()
      for(YearUse in seq(2099,2299,by=50)){#(YearUse <- seq(2099,2299,by=50)[2])
        if(!paste0(Model,"_",RCP,"_",YearUse,"_SEDminSumm.tif")%in%dir("./Results/Novelty/koeppen_geiger/")){
          # Load the Future points (and 10 yr period of conditions up to the point of interest) data for an RCP
          RCPList <- lapply(paste0(Model,"_",c(c(YearUse-9):YearUse),"_koeppen_geigerVars.tif"),
          ...
          writeRaster(MDminSumm,
                      paste0("./Results/Novelty/koeppen_geiger/",Model,"_",RCP,"_",YearUse,"_MDminSumm.tif"),
                      overwrite = TRUE)
          ...
          writeRaster(SEDminSumm,
                      paste0("./Results/Novelty/koeppen_geiger/",Model,"_",RCP,"_",YearUse,"_SEDminSumm.tif"),
                      overwrite = TRUE)
          ...
          #Save the Output
          saveRDS(Out.List,
                  paste0("./Results/Novelty/koeppen_geiger/",Model,"_",RCP,"_",YearUse,".rds"))
