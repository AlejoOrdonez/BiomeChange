...
for (RCP in c("RCP26", "RCP45", "RCP60", "RCP85")){#(RCP <- c("RCP26", "RCP45", "RCP60", "RCP85")[2])
  ...
  ClimNorm4RCPTmp <- lapply(1980:2010,
                            function(YearUse){#(YearUse <- c(1980:2010)[1])
                              rast(paste0("./Data/CMIP5/Processed/AllModelSumm/",RCP,"/BIOCLIM/AllModels_",RCP,"_",YearUse,".tif"),
                                   lyrs=BioclimVars)
                            })
  ...
  if(!paste0("AllModels_",RCP,"_TreshSumm.rds")%in%dir(paste0("./Results/Novelty/AllModels/",RCP,"/BIOCLIM/"))){
    ...
    #Save the Output
    saveRDS(Out.List,
            paste0("./Results/Novelty/AllModels/",RCP,"/BIOCLIM2/AllModels_",RCP,"_TreshSumm.rds"))
  }
    ...
    if(!paste0("AllModels_",RCP,"_",YearUse,"_SEDminSumm.tif")%in%dir(paste0("./Results/Novelty/AllModels/",RCP,"/BIOCLIM/"))){
      ...
      RCPList <- lapply(c(YearUse-9):YearUse,
                        function(YearUseTmp){#(YearUseTmp <- c(c(YearUse-9):YearUse)[1])
                          rast(paste0("./Data/CMIP5/Processed/AllModelSumm/",RCP,"/BIOCLIM/AllModels_",RCP,"_",YearUseTmp,".tif"),
                               lyrs = BioclimVars)
                        })
      ...
      writeRaster(MDminSumm,
                  paste0("./Results/Novelty/AllModels/",RCP,"/BIOCLIM2/AllModels_",RCP,"_",YearUse,"_MDminSumm.tif"),
                  overwrite = TRUE)
      ...
      writeRaster(SEDminSumm,
                  paste0("./Results/Novelty/AllModels/",RCP,"/BIOCLIM2/AllModels_",RCP,"_",YearUse,"_SEDminSumm.tif"),
                  overwrite = TRUE)      
    }
