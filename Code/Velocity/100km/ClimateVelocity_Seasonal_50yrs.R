...
for (RCP in c("RCP26", "RCP45", "RCP60", "RCP85")){#(RCP <- c("RCP26", "RCP45", "RCP60", "RCP85")[2])
  for(YearUse in seq(2049,2299,by=50)){#(YearUse <- seq(2049,2299,by=50)[1])
    ### Velocity per variable
    for(var in paste0(rep(c("P.","Tm."), each = 4),
                      c("MAP","JJA", "SON", "DJF"))){#(var <- paste0(rep(c("P.","Tm."), each = 4),c("MAP","JJA", "SON", "DJF"))[1])
      if(!paste0("AllModels_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif")%in% dir(paste0("./Results2/Velocity/AllModels/",RCP,"/Seasonal/"))){
        # split the Variable name into variable and season strings
        var2 <- strsplit(var,"[.]")[[1]]
        ...
        RCPList <- lapply(paste0("AllModels_",RCP,"_",c(YearUse-50):c(YearUse),"_",var2[1],".Seasonal.tif"),
                          function(x){#(x<- paste0("AllModels_",RCP,"_",c(YearUse-50):c(YearUse),"_",var2[1],".Seasonal.tif")[1])
                            tmp <- rast(paste0("./Data/CMIP5/Processed/AllModelSumm1Arcmin/",RCP,"/Seasonal/",x),
                                        lyrs = var2[[2]])
        ...
        ClimNormMn <- lapply(paste0("AllModels_",RCP,"_",c(YearUse-80):c(YearUse-50),"_",var2[1],".Seasonal.tif"),
                             function(x){#(x<- paste0("AllModels_",RCP,"_",c(YearUse-80):c(YearUse-50),"_",var2[1],".Seasonal.tif")[1])
                               tmp <- rast(paste0("./Data/CMIP5/Processed/AllModelSumm1Arcmin/",RCP,"/Seasonal/",x),
                                           lyrs = var2[[2]])
                               # Project and match the target resolution
                               tmp <- project(tmp,"+proj=eck4")
        ...
        ClimFut <- lapply(paste0("AllModels_",RCP,"_",c(YearUse-30):c(YearUse),"_",var2[1],".Seasonal.tif"),
                          function(x){#(x<- paste0("AllModels_",RCP,"_",c(YearUse-30):c(YearUse),"_",var2[1],".Seasonal.tif")[1])
                            tmp <- rast(paste0("./Data/CMIP5/Processed/AllModelSumm1Arcmin/",RCP,"/Seasonal/",x),
                                        lyrs = var2[[2]])
                            # Project and match the target resolution
                            tmp <- project(tmp,"+proj=eck4")
        ...
        #Save the Output
        writeRaster(OutRast,
                    filename = paste0("./Results2/Velocity/AllModels/",RCP,"/Seasonal/","AllModels_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                    overwrite = TRUE)
...
for (RCP in c("RCP26", "RCP45", "RCP60", "RCP85")){#(RCP <- c("RCP26", "RCP45", "RCP60", "RCP85")[4])
  for(YearUse in seq(2049,2299,by=50)){#(YearUse <- seq(2049,2299,by=50)[1])
    ...
    if(!paste0("AllModels_DispDiv_",RCP,"_",c(YearUse-50),"to",YearUse,".tif")%in%dir(paste0("./Results2/Velocity/AllModels/",RCP,"/Seasonal/"))){
      # Step 1a. Load the ARM derived velocities        
      Vel.ARM.List <- lapply(paste0(rep(c("P.","Tm."), each = 4),c("MAP","JJA", "SON", "DJF")),
                             function(var){#(var <- paste0(rep(c("P.","Tm."), each = 4),c("MAP","JJA", "SON", "DJF"))[1])
                               rast(paste0("./Results2/Velocity/AllModels/",RCP,"/Seasonal/","AllModels_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                                    lyrs = "Vel.ARM")
                             })
      ...
      Vel.IntAnn.List <- lapply(paste0(rep(c("P.","Tm."), each = 4),c("MAP","JJA", "SON", "DJF")),
      ...
      Vel.Ann.List <- lapply(paste0(rep(c("P.","Tm."), each = 4),c("MAP","JJA", "SON", "DJF")),
                             function(var){#(var <- names(ClimNormMn)[1])
                               rast(paste0("./Results2/Velocity/AllModels/",RCP,"/Seasonal/","AllModels_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                                    lyrs = "Vel.Ann")
                             })
      ...
      BearingList <- lapply(paste0(rep(c("P.","Tm."), each = 4),c("MAP","JJA", "SON", "DJF")),
                            function(var){#(var <- names(ClimNormMn)[1])
                              rast(paste0("./Results2/Velocity/AllModels/",RCP,"/Seasonal/","AllModels_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                                   lyrs = "Bearing")
                            })
      ...
      writeRaster(OutRastMultVar,
                  filename = paste0("./Results2/Velocity/AllModels/",RCP,"/Seasonal/","AllModels_DispDiv_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                  overwrite = TRUE)
      ## Clean the Memory
