...
for (Model in c(ModelsAll)){#(Model <- ModelsAll[[1]])
  ...
  HistFiles1980_2005 <-lapply(paste0(Model,"_",1980:2005),
                              function(x){#(x<- paste0(Model,"_",1980:2005)[1])
                                out <- c(rast(paste0("./Data/CMIP5/Processed/Historical/Seasonal/",x,"_P.Seasonal.tif")),
                                         rast(paste0("./Data/CMIP5/Processed/Historical/Seasonal/",x,"_Tm.Seasonal.tif")))
                                names(out) <- paste0(rep(c("pr.","tas."),each=4),names(out))  
                                return(out)
                              })
  ...
  for (RCP in c("RCP26", "RCP46", "RCP60", "RCP85")){#(RCP <- c("RCP26", "RCP46", "RCP60", "RCP85")[1])
    if(length(grep(Model,dir(paste0("./Data/CMIP5/Processed/",RCP,"/Seasonal"))))!=0){
      ...
      RCPList <- lapply(paste0(Model,"_",2006:2010),
                        function(x){#(x<- paste0(Model,"_",2006:2010)[1])
                          out <- c(rast(paste0("./Data/CMIP5/Processed/",RCP,"/Seasonal/",x,"_P.Seasonal.tif")),
                                   rast(paste0("./Data/CMIP5/Processed/",RCP,"/Seasonal/",x,"_Tm.Seasonal.tif")))
                          names(out) <- paste0(rep(c("pr.","tas."),each=4),names(out))  
                          return(out)  		 
                        })
      ...
      for(YearUse in seq(2099,2299,by=50)){#(YearUse <- seq(2099,2299,by=50)[3])
        ...
        RCPList <- lapply(paste0(Model,"_",c(2006:YearUse)),
        ...
        for(var in names(HistFiles1980_2005[[1]])){#(var <- names(HistFiles1980_2005[[1]])[1])
          if(!paste0(Model,"_",var,"_",RCP,"_1980to",YearUse,".tif")%in%dir("./Results/Velocity/Seasonal/")){
            ...
            #Save the Output
            writeRaster(OutRast,
                        filename = paste0("./Results/Velocity/Seasonal/",Model,"_",var,"_",RCP,"_1980to",YearUse,".tif"),
                        overwrite = TRUE)
            # clean memory
            rm(list = c("TimeSerRast",
                        "TimeHetARM","TimeHetIntAnn","TimeHetAnom",
                        "SpatHetTmp",
                        "VelocityARM","VelocityIntAnn","VelocityAnom",
                        "BearingTmp",
                        "OutRast"))
            gc()
          }
        ...
        Vel.ARM.List <- lapply(names(HistFiles1980_2005[[1]]),
                               function(var){#(var <- names(HistFiles1980_2005[[1]])[1])
                                 rast(paste0("./Results/Velocity/Seasonal/",Model,"_",var,"_",RCP,"_1980to",YearUse,".tif"),
                                      lyrs = "Vel.ARM")
                               })
        ...
        Vel.StEnAnn.List <- lapply(names(ClimNormMn),
                                   function(var){#(var <- names(ClimNormMn)[1])
                                     abs(rast(paste0("./Results/Velocity/Seasonal/",Model,"_",var,"_",RCP,"_1980to",YearUse,".tif"),
                                              lyrs = "Vel.StEnAnn"))
                                   })
        ...
        writeRaster(OutRastMultVar,
                    filename = paste0("./Results/Displacement_Divergence/Seasonal/",Model,"_DispDiv_",RCP,"_1980to",YearUse,".tif"),
                    overwrite = TRUE)
        ## Clean the Memory
        rm(list = ls()[!ls()%in%c("Model","RCP","BioclimVars","BIOMES","ModelsAll",
                                  "HistFiles1980_2005","ClimNormMn",
