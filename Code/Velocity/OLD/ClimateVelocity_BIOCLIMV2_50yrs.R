    ...
    for(YearUse in seq(2099,2299,by=50)){#(YearUse <- seq(2099,2299,by=50)[1])
      if(length(grep(Model,dir(paste0("./Data/CMIP5/Processed/",RCP,"/BIOCLIM"))))!=0){
        ...
        for(var in names(ClimNormMn)){#(var <- names(ClimNormMn)[1])
          if(!paste0(Model,"_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif")%in%dir("./Results/Velocity/BIOCLIM/")){
            ...
            #Save the Output
            writeRaster(OutRast,
                        filename = paste0("./Results/Velocity/BIOCLIM/",Model,"_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                        overwrite = TRUE)
            # clean memory
        ...
        Vel.ARM.List <- lapply(names(ClimNormMn),
                               function(var){#(var <- names(ClimNormMn)[1])
                                 rast(paste0("./Results/Velocity/BIOCLIM/",Model,"_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                                      lyrs = "Vel.ARM")
                               })
        ...
        writeRaster(OutRastMultVar,
                    filename = paste0("./Results/Displacement_Divergence/BIOCLIM/",Model,"_DispDiv_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                    overwrite = TRUE)
        ## Clean the Memory
        rm(list = ls()[!ls()%in%c("Model","RCP","BioclimVars","BIOMES","ModelsAll",
