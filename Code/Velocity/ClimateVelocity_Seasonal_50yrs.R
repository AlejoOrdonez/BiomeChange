  ...
  for (RCP in c("RCP26", "RCP46", "RCP60", "RCP85")){#(RCP <- c("RCP26", "RCP46", "RCP60", "RCP85")[1])
    for(YearUse in seq(2099,2299,by=50)){#(YearUse <- seq(2099,2299,by=50)[1])
      if(length(grep(Model,dir(paste0("./Data/CMIP5/Processed/",RCP,"/koeppen_geiger"))))!=0){
        ...
        ClimNorm4RCPTmp <- lapply(paste0(Model,"_",c(YearUse-80):c(YearUse-50)),
        ...
        names(ClimNormMn) <- c("pr.MAP", "pr.JJA", "pr.SON", "pr.DJF", "tas.MAP", "tas.JJA", "tas.SON", "tas.DJF")
        #Crop oceans
        ...
        # Load the 50Yrs of values for an RCP
        RCPList <- lapply(paste0(Model,"_",c(c(YearUse-50):YearUse)),
                          function(x){#(x<- paste0(Model,"_",c(c(YearUse-50):YearUse))[1])
                            out <- c(rast(paste0("./Data/CMIP5/Processed/",RCP,"/Seasonal/",x,"_P.Seasonal.tif")),
                                     rast(paste0("./Data/CMIP5/Processed/",RCP,"/Seasonal/",x,"_Tm.Seasonal.tif")))
                            names(out) <- paste0(rep(c("pr.","tas."),each=4),names(out))  
                            return(out)
        ...
        for(var in names(ClimNormMn)){#(var <- names(ClimNormMn)[1])
          if(!paste0(Model,"_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif")%in%dir("./Results/Velocity/Seasonal/")){
            ...
            #Save the Output
            writeRaster(OutRast,
                        filename = paste0("./Results/Velocity/Seasonal/",Model,"_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                        overwrite = TRUE)
            # clean memory
        ...
        Vel.ARM.List <- lapply(names(ClimNormMn),
                               function(var){#(var <- names(ClimNormMn)[1])
                                 rast(paste0("./Results/Velocity/Seasonal/",Model,"_",var,"_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                                      lyrs = "Vel.ARM")
                               })
        ...
        writeRaster(OutRastMultVar,
                    filename = paste0("./Results/Displacement_Divergence/Seasonal/",Model,"_DispDiv_",RCP,"_",c(YearUse-50),"to",YearUse,".tif"),
                    overwrite = TRUE)
        ## Clean the Memory
        rm(list = ls()[!ls()%in%c("Model","RCP","koeppen_geigerVars","BIOMES","ModelsAll",
