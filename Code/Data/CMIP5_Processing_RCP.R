...
for(Model in dir(WD.Raw)){#(Model <- dir(WD.Raw)[1])#"HadGEM2-ES")#"CCSM4")#
  for (RCP in c("RCP26","RCP45","RCP60","RCP85")){#(RCP <- "RCP45")
    if(RCP%in%dir(paste0(WD.Raw,"/",Model))){
      setwd(paste0(WD.Raw,"/",Model,"/",RCP)) # Define the Directory with the Model/RCP combination
      # Set the target Year
      for(TargYr in 2006:2299){#TargYr <- 2007
        if(!paste0(Model,"_",TargYr,"_koeppen_geigerVars.tif")%in%dir(paste0(WD.Processed,"/",RCP,"/koeppen_geiger/"))){
          a <-Sys.time()
          ...
          rm(PList);gc()
          terra::rotate(x = P,
                        filename = paste0(WD.Processed,"/",RCP,"/Monthly/",Model,"_",TargYr,"_pr.tif"),
                        overwrite = TRUE)
          # Load climatology for a target Year - Minimum Temperature Data
          TnList <- lapply(dir(pattern="tasmin_"),
                           function(i){
                             rast(i)
                           })
          ...
          rm(TnList);gc()
          terra::rotate(x = Tn,
                        filename = paste0(WD.Processed,"/",RCP,"/Monthly/",Model,"_",TargYr,"_tasmin.tif"),
                        overwrite = TRUE)
          
          # Load climatology for a target Year - Maximum Temperature Data
          if(length(dir(pattern="tasmax_"))!=0){
            TxList <- lapply(dir(pattern="tasmax_"),
                             function(i){
                               rast(i)
                             })
            ...
            rm(TxList);gc()
            terra::rotate(x = Tx,
                          filename = paste0(WD.Processed,"/",RCP,"/Monthly/",Model,"_",TargYr,"_tasmax.tif"),
                          overwrite = TRUE)
          }
          
          # Load climatology for a target Year - Mean Temperature Data
          if(length(dir(pattern="tas_"))!=0){
            ...
            rm(TmList);gc()
            terra::rotate(x = Tm,
                          filename = paste0(WD.Processed,"/",RCP,"/Monthly/",Model,"_",TargYr,"_tas.tif"),
                          overwrite = TRUE)
          }
          # If there is no Mean Temperature Data (in CCSM4) use a symmetry approximation
          if(length(dir(pattern="tas_"))==0){
            Tm <- (Tn+Tx)/2
            terra::rotate(x = Tm,
                          filename = paste0(WD.Processed,"/",RCP,"/Monthly/",Model,"_",TargYr,"_tas.tif"),
          ...
          # If there is no Tmax (in HadGEM2-ES) use a symmetry approximation
          if(length(dir(pattern="tasmax_"))==0){
            Tx <- Tm + (Tm - Tn)
            terra::rotate(x = Tx,
                          filename = paste0(WD.Processed,"/",RCP,"/Monthly/",Model,"_",TargYr,"_tasmax.tif"),
          ...
          terra::rotate(x = BIOCLIM.Vars2,
                        filename = paste0(WD.Processed,"/",RCP,"/BIOCLIM/",Model,"_",TargYr,"_BIOCLIM.tif"),
                        overwrite = TRUE)
          ...
          terra::rotate(x = P.Seasonal,
                        filename = paste0(WD.Processed,"/",RCP,"/Seasonal/",Model,"_",TargYr,"_P.Seasonal.tif"),
                        overwrite = TRUE)
          ...
          terra::rotate(x = Tm.Seasonal,
                        filename = paste0(WD.Processed,"/",RCP,"/Seasonal/",Model,"_",TargYr,"_Tm.Seasonal.tif"),
                        overwrite = TRUE)
          ...
          terra::rotate(x = koeppen_geigerVars,
                        filename = paste0(WD.Processed,"/",RCP,"/koeppen_geiger/",Model,"_",TargYr,"_koeppen_geigerVars.tif"),
                        overwrite = TRUE)
          
          a - Sys.time()
          # Clean memory
          rm(list=ls()[!ls()%in%c("WD.Raw","WD.Processed","Model","RCP","TargYr")]);gc()  
        }
      }
    }
  }
