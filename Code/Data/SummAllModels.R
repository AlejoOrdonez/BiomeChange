      ...
      AllModSummList <- sfLapply(1980:2299,
                                 function(YearUse){#(YearUse <- c(1980:2299)[[1]])
                                   # Assess if the value exists
                                     ...
                                     AllModListPerYr <- lapply(unique(sapply(strsplit(dir(paste0("./Data/CMIP5/Processed/",RCP,"/",VarGroup)),"_"),function(x){x[[1]]})),
                                     ...
                                     writeRaster(AllModPerYrSumm,
                                                 paste0("./Data/CMIP5/Processed/AllModelSumm/",RCP,"/",VarGroup,"/AllModels_",RCP,"_",YearUse,".tif"),
                                                 overwrite=TRUE)
        ...
        AllModSummList <- sfLapply(1980:2299,
                                   function(YearUse){#(YearUse <- c(1980:2299)[[1]])
                                     # Assess if the value exists
                                       ...
                                       AllModListPerYr <- lapply(unique(sapply(strsplit(dir(paste0("./Data/CMIP5/Processed/",RCP,"/",VarGroup)),"_"),function(x){x[[1]]})),
                                                                 function(Model){#(Model <- unique(sapply(strsplit(dir(paste0("./Data/CMIP5/Processed/",RCP,"/",VarGroup)),"_"),function(x){x[[1]]}))[1])
                                                                   resample(x = rast(paste0("./Data/CMIP5/Processed/",
                                                                                            ifelse(YearUse<=2005,"Historical",RCP),
                                       ...
                                       # Save the Mean for a year summary 
                                       writeRaster(AllModPerYrSumm,
                                                   paste0("./Data/CMIP5/Processed/AllModelSumm/",RCP,"/",VarGroup,"/AllModels_",RCP,"_",YearUse,"_",VarName,".tif"),
                                                   overwrite=TRUE)
